define(['jquery', 'bootstrap', 'plupload', 'template'], function ($, undefined, Plupload, Template) {
  var Upload = {
	...
	config: {
	  ...
	  previewVideoTpl: '<li class="col-xs-3"><a href="<%=fullurl%>" data-url="<%=url%>" target="_blank" class="thumbnail"><video src="<%=fullurl%>" class="img-responsive" style="width: 80px; height: 80px;"></video></a><a href="javascript:;" class="btn btn-danger btn-xs btn-trash"><i class="fa fa-trash"></i></a></li>',
	},
	events: {
	  //初始化完成
		  ...
		  if (preview_id && input_id) {
			$(document.body).on("keyup change", "#" + input_id, function () {
			  ...
			  $.each(inputArr, function (i, j) {
				...
				if (j.split('.')[1] == 'mp4') {
				  html = tpl ? Template(tpl, data) : Template.render(Upload.config.previewVideoTpl, data);
				}
